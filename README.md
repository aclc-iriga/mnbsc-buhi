...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/mnbsc-buhi`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `mnbsc-buhi` and import [mnbsc-buhi.sql](mnbsc-buhi.sql) into it.

6. Open the terminal and navigate to the project directory **mnbsc-buhi**.
...
9. Open your web browser and access <http://localhost:5201/mnbsc-buhi/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-mnbsc-buhi` and import [mnbsc-buhi.sql](mnbsc-buhi.sql) into it.
...
2. Access the application by visiting `http://[host_name]/mnbsc-buhi`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/mnbsc-buhi>
     - <http://192.168.1.99/mnbsc-buhi>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/mnbsc-buhi/app>.
